...
const MARKETS = [
  ...
  { code: 'DE', base: 'https://www.dm.de', cities: ['Berlin', 'Hamburg', 'München', 'Köln', 'Frankfurt', 'Stuttgart', 'Leipzig', 'Dortmund', 'Bremen', 'Nürnberg'] },
...
const crawler = new PlaywrightCrawler({
  ...
  async requestHandler({ request, page }) {
    ...
    const cookieButtons = page.locator('button').filter({ hasText: COOKIE_REJECT });
    ...
    const origin = await page.locator('details').evaluateAll((details) => details.map((item) => ({
      heading: item.querySelector('summary')?.textContent?.replace(/\s+/g, ' ').trim() ?? '',
      value: item.querySelector('[data-dmid$="-content"]')?.textContent?.replace(/\s+/g, ' ').trim() ?? '',
    })));
    ...
    const availabilityButton = page.locator('button').filter({ hasText: AVAILABILITY_BUTTON }).first();
    if (!(await availabilityButton.count())) return;
    await availabilityButton.click();

    const dialog = page.locator('[role="dialog"]').last();
    await dialog.waitFor({ state: 'visible', timeout: 15000 });
    const searchInput = dialog.locator('input[type="search"]').first();
    const collected = new Map();

    for (const cityQuery of market.cities) {
      if (outputRows + collected.size >= maxOutputRows) break;
      await searchInput.fill(cityQuery);
      const searchButtons = searchInput.locator('xpath=..').locator('button');
      if (await searchButtons.count()) await searchButtons.last().click();
      ...
      const stores = await dialog.locator('[data-dmid="store-teaser"]').evaluateAll((cards, limit) => cards.slice(0, limit).map((card) => {
        ...
        const quantity = Number((availability.match(/(\d+)/) ?? [])[1] ?? 0);
        return {
          available: Boolean(card.querySelector('[data-dmid="store-available-icon"]')) && quantity > 0,
          quantity,
          availability,
    ...
    const rows = [...collected.values()].slice(0, Math.max(0, maxOutputRows - outputRows)).map((store) => ({
